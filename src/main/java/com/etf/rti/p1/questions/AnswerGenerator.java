...
public class AnswerGenerator {
    ...
    public String generateAnswer(Node<Symbol> node, int len) {
        ...
        if (randNode.getData().isComposite()) {
            ...
            while (!nonterminals.isEmpty()) {
                ...
                int i = 0;    // ako u listi imamo jedan nerekurzivni, prvo treba odabrati njega - NE RESAVAMO PROBLEM AKO IMAMO VISE OD JEDNOG NEREKURZIVNOG - TO TREBA RESITI!
            ...
            return generateAnswerForCompositeNode(nodes, pairs);
        } else {
    ...
    private String generateAnswerForCompositeNode(List<Node<Symbol>> nodes, List<Pair<Node<Symbol>, Integer>> pairs) {
        ...
        for (int i = 0; i < nodes.size(); i++) {
            if (nodes.get(i) == null) {
                answer += generateAnswer(pairs.get(i).getLeft(), pairs.get(i).getRight());
            } else {
    ...
    public List<Node<Symbol>> getListOfNodes(Node<Symbol> node) {
        ...
        Pattern patt = Pattern.compile(TERMINALS_NONTERMINALS);
    ...
    private String swapTerminals(String correctAnswer, List<Pair<Node<Symbol>, Node<Symbol>>> pairs, int correctLen, int answerLen) {
        String corruptAnswer = correctAnswer;
        for (int i = correctLen; i <= answerLen; i++) {
            for (Pair<Node<Symbol>, Node<Symbol>> pair : pairs) {
                List<String> terminals1 = new ArrayList<>();
                List<String> terminals2 = new ArrayList<>();
                getChildrenTerminals(pair.getLeft(), terminals1);
                getChildrenTerminals(pair.getRight(), terminals2);
